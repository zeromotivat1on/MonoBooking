...
import IUpdateCompany from '../../common/interfaces/Company/IUpdateCompany';
...
export default class UpdateCompany extends Component {
  ...
  getFormFields() {
    const inputs = document.querySelectorAll('#update-company-form input');
  ...
  validateName(field: HTMLInputElement | HTMLTextAreaElement) {
    if ((field.value.length < 4 || field.value.length > 64)) { this.invalidField(field); }
  ...
  validateDescription(field: HTMLInputElement | HTMLTextAreaElement) {
    if ((field.value.length < 10 || field.value.length > 200)) { this.invalidField(field); }
    else { this.correctField(field); }
  }
  ...
  async submitForm(e: FormEvent) {
    e.preventDefault();
    let response = await this.httpService.put(this.state.company, `${apiUrl.Company}/${this.lastPartOfUrl}`);
    console.log(response);
  }

  render() {
    return (
      <main id='update-company-page' className='def-page'>
        {this.state.company.id > 0 ? (
          <Form onSubmit={(e) => this.submitForm(e)} id='update-company-form'>
            <FormGroup>
            ...
            <FormGroup>
              ...
              <Input required type='text' name='owner' id='owner'
                defaultValue={this.state.company.owner}
                onChange={(e) => this.formChangeHandler(e)} />
            </FormGroup>
            <FormGroup>
              <Label for='description'>Description</Label>
              <Input required type='textarea' rows={5} name='description' id='description'
                defaultValue={this.state.company.description}
                onChange={(e) => this.formChangeHandler(e)} />
            </FormGroup>
            <FormGroup>
              <Label for='daysOnSite'>Days On Site</Label>
              <Input type='number' name='daysOnSite' id='daysOnSite'
                defaultValue={this.state.company.daysOnSite}
                onChange={(e) => this.formChangeHandler(e)} />
            </FormGroup>
            <FormGroup>
              ...
              <Input type='date' name='foundationDate' id='foundationDate'
                max={getCurrentDate()}
                defaultValue={this.state.company.foundationDate?.substr(0, 10)}
                onChange={(e) => this.formChangeHandler(e)} />
            </FormGroup>
            <FormGroup>
              ...
              <Input type='date' name='lastModifiedDate' id='lastModifiedDate'
                max={getCurrentDate()}
                defaultValue={this.state.company.lastModifiedDate?.substr(0, 10)}
                onChange={(e) => this.formChangeHandler(e)} />
            </FormGroup>
            <FormGroup>
              <Label for='status'>Status</Label>
              <Input type='select' name='status' id='status'
                defaultValue={this.state.company.status}
                onChange={(e) => this.formChangeHandler(e)}>
            ...
            <FormGroup check>
              <Label check>
                <Input type='checkbox' name='deleted' id='deleted'
                  defaultValue={this.state.company.deleted.toString()}
                  onChange={(e) => this.formChangeHandler(e)} />{' '}
                Deleted
              </Label>
            </FormGroup>
